...
public interface IndexRepository extends JpaRepository<IndexEntity, Integer> {
    ...
    @Transactional
    @Query("SELECT i.pageEntity.id FROM IndexEntity i WHERE i.lemmaEntity = :lemmaEntity")
    List<Integer> getPageEntityByLemma(@Param("lemmaEntity") LemmaEntity lemmaEntity);

    @Transactional
    @Query("SELECT MAX(i.rank) FROM IndexEntity i WHERE i.lemmaEntity IN (:lemmaEntityList)")
